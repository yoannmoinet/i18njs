describe('i18njs', function () {
	...
	it('should add locales to the dictionary', function () {
		...
		expect(i18n.getDico().en.ns.eval).to.eql('{{for(var i = 0, max = 3; i < 3; i += 1) {}}eval {{}}}');
		...
		expect(i18n.getDico().fr.ns.eval).to.eql('{{for(var i = 0, max = 3; i < 3; i += 1) {}}eval {{}}}');
	...
	it('should escape characters', function () {
		expect(i18n.get('ns.escape', {escape: '&<>"\'`'})).to.eql('&amp;&lt;&gt;&quot;&#x27;&#x60;');
	});
	...
	it('should default when not found', function () {
		expect(i18n.get('test.on.inexistant.string')).to.eql('test.on.inexistant.string');
	});
